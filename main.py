from func import get_hh_vacancies, convert_sql, create_database, save_data_database
...
def main():
    ...
    while user_choice != 'q':
        if user_choice == 'a':
            ...
            db.get_companies_and_vacancies_count('database_HH', params)
            user_choice = input('Выберите следующую команду ')
        if user_choice == 'b':
            #вывод списка всех вакансий с указанием названия компании,
            #названия вакансии, зарплаты и ссылки на вакансию
            db.get_all_vacancies('database_HH', params)
            user_choice = input('Выберите следующую команду ')
        if user_choice == 'c':
            #вывод средней зарплаты по вакансиям
            db.get_avg_salary('database_HH', params)
            user_choice = input('Выберите следующую команду ')
        if user_choice == 'd':
            #вывод списка всех вакансий, у которых зарплата выше средней по всем вакансиям
            db.get_vacancies_with_higher_salary('database_HH', params)
            user_choice = input('Выберите следующую команду ')
        if user_choice == 'e':
            db.get_vacancies_with_keyword('database_HH', params)
            user_choice = input('Выберите следующую команду ')
        else:
